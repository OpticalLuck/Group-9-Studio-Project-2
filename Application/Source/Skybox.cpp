#include "Skybox.h"
Skybox::Skybox(GOManager gomanager, MeshList* meshlist)
{			 
	SB_ARR[SBX_TOP] = gomanager.CreateGO<GameObject>(meshlist->GetMesh(MeshList::SKYBOX_TOP));
	SB_ARR[SBX_TOP]->SetTranslate(Vector3(0, 50, 0));
	...
	SB_ARR[SBX_BOTTOM]->SetTranslate(Vector3(0, -50, 0));
	
	SB_ARR[SBX_LEFT] = gomanager.CreateGO<GameObject>(meshlist->GetMesh(MeshList::SKYBOX_LEFT));
	SB_ARR[SBX_LEFT]->SetTranslate(Vector3(-50, 0, 0));
	...
	SB_ARR[SBX_RIGHT]->SetTranslate(Vector3(50, 0, 0));
	...
	SB_ARR[SBX_BACK]->SetTranslate(Vector3(0, 0, 50));
	...
	SB_ARR[SBX_FRONT]->SetTranslate(Vector3(0, 0, -50));
